...
public class LoginCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		// 전처리
		// 1. session에 userVo(LoginServlet에서 정상 로그인 처리 되었을 시 저장한 속성)가 있는지 확인(정상적으로 로그인한 상태인지 확인)
		...
		// ** 모든 페이지에 대해 적용 해야하나??
		// session이 필요한 페이지랑, 필요없는 페이지를 구분할 필요가 있다.
		// session 검증이 필요없는 요청 (대다수는 검증이 필요함)
		...
		// ex) localhost/login
			...
			if(uri.equals("/login") || uri.endsWith(".js") || uri.endsWith(".css") || uri.endsWith(".jpg") || uri.endsWith(".jpeg") || uri.endsWith(".gif") || uri.endsWith(".png"))
				chain.doFilter(request, response);
			else
